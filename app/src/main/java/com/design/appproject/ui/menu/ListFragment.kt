...
import com.design.appproject.ui.menu.ListFilterDialog
...
import com.design.appproject.databinding.MenucommonListLayoutBinding
import com.design.appproject.logic.repository.HomeRepository
import com.design.appproject.logic.viewmodel.menu.ListModel
import com.design.appproject.ui.menu.ListAdapter
 ...
 * 菜单列表页
 */
@Route(path = CommonArouteApi.PATH_FRAGMENT_LIST_MENU)
class ListFragment : BaseBindingFragment<MenucommonListLayoutBinding>() {
    ...
    override fun initEvent() {
        setBarTitle("菜单",mHasBack)
        setBarColor("#C6EBF1","black")
        binding.apply {
            addBtn.isVisible = (mIsBack && Utils.isAuthBack("menu","新增"))|| Utils.isAuthFront("menu","新增")
            addBtn.setOnClickListener {
                ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_MENU).navigation()
            }
          ...
          mListAdapter.addChildClickViewIds(R.id.edit1_tv,R.id.edit2_left_tv,R.id.edit2_right_tv
            ...
            mListAdapter.setOnItemChildClickListener { adapter, view, position ->
                when(view.id){
                    ...
                    R.id.edit3_tv,R.id.edit4_left_tv,R.id.edit4_right_tv->ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_MENU)
                    ...
                    R.id.delete3_tv,R.id.delete4_left_tv,R.id.delete4_right_tv->XPopup.Builder(requireActivity()).asConfirm("提示","是否确认删除") {
                        HomeRepository.delete<Any>("menu",listOf(mListAdapter.data[position][when(view.id){
                    ...
                    R.id.list_box3_fbl, R.id.list_box4_left_fbl, R.id.list_box4_right_fbl,->ArouterUtils.startFragment(CommonArouteApi.PATH_FRAGMENT_DETAILS_MENU,map=mapOf("mId" to mListAdapter.data[position][when(view.id){
    ...
    private val lists = mutableListOf<List<MenuItemBean>>()

    //添加list集合到lists
    private fun addLists(list: List<MenuItemBean>) {
        var lastList = mutableListOf<MenuItemBean>()
        if (lists.isNotEmpty()) {
    ...
    private fun loadData(page:Int,searchParams:MutableMap<String,String>?=null){
        ...
        mIsBack.yes {
                mListModel.page("menu", requestParams)
        }.otherwise {
                mListModel.list("menu", requestParams)
        }
    }
}