...
import com.design.appproject.ui.discussfuwuxiangmu.ListFilterDialog
...
import com.design.appproject.databinding.DiscussfuwuxiangmucommonListLayoutBinding
import com.design.appproject.logic.repository.HomeRepository
import com.design.appproject.logic.viewmodel.discussfuwuxiangmu.ListModel
import com.design.appproject.ui.discussfuwuxiangmu.ListAdapter
 ...
 * 服务项目评论表列表页
 */
@Route(path = CommonArouteApi.PATH_FRAGMENT_LIST_DISCUSSFUWUXIANGMU)
class ListFragment : BaseBindingFragment<DiscussfuwuxiangmucommonListLayoutBinding>() {
    ...
    override fun initEvent() {
        setBarTitle("服务项目评论表",mHasBack)
        setBarColor("#C6EBF1","black")
        binding.apply {
            addBtn.isVisible = (mIsBack && Utils.isAuthBack("discussfuwuxiangmu","新增"))|| Utils.isAuthFront("discussfuwuxiangmu","新增")
            addBtn.setOnClickListener {
                ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_DISCUSSFUWUXIANGMU).navigation()
            }
            initSearch()
          ...
          mListAdapter.addChildClickViewIds(R.id.edit1_tv,R.id.edit2_left_tv,R.id.edit2_right_tv
            ...
            mListAdapter.setOnItemChildClickListener { adapter, view, position ->
                when(view.id){
                    ...
                    R.id.edit3_tv,R.id.edit4_left_tv,R.id.edit4_right_tv->ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_DISCUSSFUWUXIANGMU)
                    ...
                    R.id.delete3_tv,R.id.delete4_left_tv,R.id.delete4_right_tv->XPopup.Builder(requireActivity()).asConfirm("提示","是否确认删除") {
                        HomeRepository.delete<Any>("discussfuwuxiangmu",listOf(mListAdapter.data[position][when(view.id){
                    ...
                    R.id.list_box3_fbl, R.id.list_box4_left_fbl, R.id.list_box4_right_fbl,->ArouterUtils.startFragment(CommonArouteApi.PATH_FRAGMENT_DETAILS_DISCUSSFUWUXIANGMU,map=mapOf("mId" to mListAdapter.data[position][when(view.id){
    ...
    private fun DiscussfuwuxiangmucommonListLayoutBinding.initSearch() {
    ...
    private val lists = mutableListOf<List<DiscussfuwuxiangmuItemBean>>()

    //添加list集合到lists
    private fun addLists(list: List<DiscussfuwuxiangmuItemBean>) {
        var lastList = mutableListOf<DiscussfuwuxiangmuItemBean>()
        if (lists.isNotEmpty()) {
    ...
    private fun loadData(page:Int,searchParams:MutableMap<String,String>?=null){
        ...
        mIsBack.yes {
                mListModel.page("discussfuwuxiangmu", requestParams)
        }.otherwise {
                mListModel.list("discussfuwuxiangmu", requestParams)
        }
    }
}