...
import com.design.appproject.ui.dianpu.ListFilterDialog
...
import com.design.appproject.databinding.DianpucommonListLayoutBinding
import com.design.appproject.logic.repository.HomeRepository
import com.design.appproject.logic.viewmodel.dianpu.ListModel
import com.design.appproject.ui.dianpu.ListAdapter
 ...
 * 店铺列表页
 */
@Route(path = CommonArouteApi.PATH_FRAGMENT_LIST_DIANPU)
class ListFragment : BaseBindingFragment<DianpucommonListLayoutBinding>() {
    ...
    val queryIndex = Pair<String,String>("店铺名称","dianpumingcheng")/*查询索引*/

    override fun initEvent() {
        setBarTitle("店铺",mHasBack)
        setBarColor("#C6EBF1","black")
        binding.apply {
            addBtn.isVisible = (mIsBack && Utils.isAuthBack("dianpu","新增"))|| Utils.isAuthFront("dianpu","新增")
            addBtn.setOnClickListener {
                ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_DIANPU).navigation()
            }
            initSearch()
          ...
          mListAdapter.addChildClickViewIds(R.id.edit1_tv,R.id.edit2_left_tv,R.id.edit2_right_tv
            ...
            mListAdapter.setOnItemChildClickListener { adapter, view, position ->
                when(view.id){
                    ...
                    R.id.edit3_tv,R.id.edit4_left_tv,R.id.edit4_right_tv->ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_DIANPU)
                    ...
                    R.id.delete3_tv,R.id.delete4_left_tv,R.id.delete4_right_tv->XPopup.Builder(requireActivity()).asConfirm("提示","是否确认删除") {
                        HomeRepository.delete<Any>("dianpu",listOf(mListAdapter.data[position][when(view.id){
                    ...
                    R.id.list_box3_fbl, R.id.list_box4_left_fbl, R.id.list_box4_right_fbl,->ArouterUtils.startFragment(CommonArouteApi.PATH_FRAGMENT_DETAILS_DIANPU,map=mapOf("mId" to mListAdapter.data[position][when(view.id){
    ...
    private fun DianpucommonListLayoutBinding.initSearch() {
    ...
    private val lists = mutableListOf<List<DianpuItemBean>>()

    //添加list集合到lists
    private fun addLists(list: List<DianpuItemBean>) {
        var lastList = mutableListOf<DianpuItemBean>()
        if (lists.isNotEmpty()) {
    ...
    private fun loadData(page:Int,searchParams:MutableMap<String,String>?=null){
        ...
        binding.searchValueEt.text.toString().isNotNullOrEmpty().yes {
            params.put("dianpumingcheng","%" + binding.searchValueEt.text.toString() + "%" )
        }.otherwise {
            params.remove("dianpumingcheng")
        }
        val requestParams = mutableMapOf<String,String>()
        requestParams.putAll(params)
        searchParams?.let { requestParams.putAll(it) }
        mIsBack.yes {
                mListModel.page("dianpu", requestParams)
        }.otherwise {
                mListModel.list("dianpu", requestParams)
        }
    }
}