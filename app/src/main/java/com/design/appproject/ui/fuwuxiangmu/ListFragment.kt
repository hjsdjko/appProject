...
import com.design.appproject.ui.fuwuxiangmu.ListFilterDialog
...
import com.design.appproject.databinding.FuwuxiangmucommonListLayoutBinding
import com.design.appproject.logic.repository.HomeRepository
import com.design.appproject.logic.viewmodel.fuwuxiangmu.ListModel
import com.design.appproject.ui.fuwuxiangmu.ListAdapter
 ...
 * 服务项目列表页
 */
@Route(path = CommonArouteApi.PATH_FRAGMENT_LIST_FUWUXIANGMU)
class ListFragment : BaseBindingFragment<FuwuxiangmucommonListLayoutBinding>() {
    ...
    override fun initEvent() {
        setBarTitle("服务项目",mHasBack)
        setBarColor("#C6EBF1","black")
        binding.apply {
            addBtn.isVisible = (mIsBack && Utils.isAuthBack("fuwuxiangmu","新增"))|| Utils.isAuthFront("fuwuxiangmu","新增")
            addBtn.setOnClickListener {
                ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_FUWUXIANGMU).navigation()
            }
            initSearch()
            initCategory()
          ...
          mListAdapter.addChildClickViewIds(R.id.edit1_tv,R.id.edit2_left_tv,R.id.edit2_right_tv
            ...
            mListAdapter.setOnItemChildClickListener { adapter, view, position ->
                when(view.id){
                    ...
                    R.id.edit3_tv,R.id.edit4_left_tv,R.id.edit4_right_tv->ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_FUWUXIANGMU)
                    ...
                    R.id.delete3_tv,R.id.delete4_left_tv,R.id.delete4_right_tv->XPopup.Builder(requireActivity()).asConfirm("提示","是否确认删除") {
                        HomeRepository.delete<Any>("fuwuxiangmu",listOf(mListAdapter.data[position][when(view.id){
                    ...
                    R.id.list_box3_fbl, R.id.list_box4_left_fbl, R.id.list_box4_right_fbl,->ArouterUtils.startFragment(CommonArouteApi.PATH_FRAGMENT_DETAILS_FUWUXIANGMU,map=mapOf("mId" to mListAdapter.data[position][when(view.id){
    ...
    private fun FuwuxiangmucommonListLayoutBinding.initSearch() {
    ...
    private fun FuwuxiangmucommonListLayoutBinding.initCategory() {
        mIsBack.yes {
            HomeRepository.page<FuwuleixingItemBean>("fuwuleixing", mapOf("page" to "1", "limit" to "100")).observeKt(errorBlock = {binding.srv.isRefreshing =false}) {
            it.getOrNull()?.let {
                setNavigator(tab,it.data.list)
            }
        } }.otherwise {
            HomeRepository.list<FuwuleixingItemBean>("fuwuleixing", mapOf("page" to "1", "limit" to "100")).observeKt(errorBlock = {binding.srv.isRefreshing =false}) {
            it.getOrNull()?.let {
                setNavigator(tab,it.data.list)
            }
        }
    ...
    private fun setNavigator(tab: LinearLayout,list:List<FuwuleixingItemBean>){
        mCategoryList.addAll(list.map{it.fuwuleixing})
    ...
    private val lists = mutableListOf<List<FuwuxiangmuItemBean>>()

    //添加list集合到lists
    private fun addLists(list: List<FuwuxiangmuItemBean>) {
        var lastList = mutableListOf<FuwuxiangmuItemBean>()
        if (lists.isNotEmpty()) {
    ...
    private fun loadData(page:Int,searchParams:MutableMap<String,String>?=null){
        ...
        if(mCategoryList[mSelectedIndex]!="全部"){
            params.put("fuwuleixing","%" + mCategoryList[mSelectedIndex] + "%" )
        }else{
            params.remove("fuwuleixing")
        }
        val requestParams = mutableMapOf<String,String>()
        requestParams.putAll(params)
        searchParams?.let { requestParams.putAll(it) }
        mIsBack.yes {
                mListModel.page("fuwuxiangmu", requestParams)
        }.otherwise {
                mListModel.list("fuwuxiangmu", requestParams)
        }
    }
}