...
import com.design.appproject.ui.fuwuleixing.ListFilterDialog
...
import com.design.appproject.databinding.FuwuleixingcommonListLayoutBinding
import com.design.appproject.logic.repository.HomeRepository
import com.design.appproject.logic.viewmodel.fuwuleixing.ListModel
import com.design.appproject.ui.fuwuleixing.ListAdapter
 ...
 * 服务类型列表页
 */
@Route(path = CommonArouteApi.PATH_FRAGMENT_LIST_FUWULEIXING)
class ListFragment : BaseBindingFragment<FuwuleixingcommonListLayoutBinding>() {
    ...
    val queryIndex = Pair<String,String>("服务类型","fuwuleixing")/*查询索引*/

    override fun initEvent() {
        setBarTitle("服务类型",mHasBack)
        setBarColor("#C6EBF1","black")
        binding.apply {
            addBtn.isVisible = (mIsBack && Utils.isAuthBack("fuwuleixing","新增"))|| Utils.isAuthFront("fuwuleixing","新增")
            addBtn.setOnClickListener {
                ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_FUWULEIXING).navigation()
            }
            initSearch()
          ...
          mListAdapter.addChildClickViewIds(R.id.edit1_tv,R.id.edit2_left_tv,R.id.edit2_right_tv
            ...
            mListAdapter.setOnItemChildClickListener { adapter, view, position ->
                when(view.id){
                    ...
                    R.id.edit3_tv,R.id.edit4_left_tv,R.id.edit4_right_tv->ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_FUWULEIXING)
                    ...
                    R.id.delete3_tv,R.id.delete4_left_tv,R.id.delete4_right_tv->XPopup.Builder(requireActivity()).asConfirm("提示","是否确认删除") {
                        HomeRepository.delete<Any>("fuwuleixing",listOf(mListAdapter.data[position][when(view.id){
                    ...
                    R.id.list_box3_fbl, R.id.list_box4_left_fbl, R.id.list_box4_right_fbl,->ArouterUtils.startFragment(CommonArouteApi.PATH_FRAGMENT_DETAILS_FUWULEIXING,map=mapOf("mId" to mListAdapter.data[position][when(view.id){
    ...
    private fun FuwuleixingcommonListLayoutBinding.initSearch() {
    ...
    private val lists = mutableListOf<List<FuwuleixingItemBean>>()

    //添加list集合到lists
    private fun addLists(list: List<FuwuleixingItemBean>) {
        var lastList = mutableListOf<FuwuleixingItemBean>()
        if (lists.isNotEmpty()) {
    ...
    private fun loadData(page:Int,searchParams:MutableMap<String,String>?=null){
        ...
        binding.searchValueEt.text.toString().isNotNullOrEmpty().yes {
            params.put("fuwuleixing","%" + binding.searchValueEt.text.toString() + "%" )
        }.otherwise {
        ...
        mIsBack.yes {
                mListModel.page("fuwuleixing", requestParams)
        }.otherwise {
                mListModel.list("fuwuleixing", requestParams)
        }
    }
}