...
class MineFragment: BaseBindingFragment<FragmentMineLayoutBinding>() {
    ...
    private fun FragmentMineLayoutBinding.setData(session:LinkedTreeMap<String, Any>){
        ...
        dianpuLl.isVisible = "dianpu" == CommonBean.tableName
        dianpudianpumingchengTv.text =session["dianpumingcheng"].toString()+if (session["vip"].toString()=="是")"(VIP)" else ""
        dianpudianputupianIfv.isVisible = "dianpu" == CommonBean.tableName
        activity?.let {dianpudianputupianIfv.load(it,session["dianputupian"].toString(), errorResId = R.mipmap.upload) }
        fuwurenyuanLl.isVisible = "fuwurenyuan" == CommonBean.tableName
        fuwurenyuanyuangongxingmingTv.text =session["yuangongxingming"].toString()+if (session["vip"].toString()=="是")"(VIP)" else ""
        fuwurenyuantouxiangIfv.isVisible = "fuwurenyuan" == CommonBean.tableName
        activity?.let {fuwurenyuantouxiangIfv.load(it,session["touxiang"].toString(), errorResId = R.mipmap.upload) }
    }
}