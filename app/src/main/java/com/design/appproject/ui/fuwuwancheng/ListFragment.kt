...
import com.design.appproject.ui.fuwuwancheng.ListFilterDialog
...
import com.design.appproject.databinding.FuwuwanchengcommonListLayoutBinding
import com.design.appproject.logic.repository.HomeRepository
import com.design.appproject.logic.viewmodel.fuwuwancheng.ListModel
import com.design.appproject.ui.fuwuwancheng.ListAdapter
 ...
 * 服务完成列表页
 */
@Route(path = CommonArouteApi.PATH_FRAGMENT_LIST_FUWUWANCHENG)
class ListFragment : BaseBindingFragment<FuwuwanchengcommonListLayoutBinding>() {
    ...
    override fun initEvent() {
        setBarTitle("服务完成",mHasBack)
        setBarColor("#C6EBF1","black")
        binding.apply {
            addBtn.isVisible = (mIsBack && Utils.isAuthBack("fuwuwancheng","新增"))|| Utils.isAuthFront("fuwuwancheng","新增")
            addBtn.setOnClickListener {
                ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_FUWUWANCHENG).navigation()
            }
            initSearch()
          ...
          mListAdapter.addChildClickViewIds(R.id.edit1_tv,R.id.edit2_left_tv,R.id.edit2_right_tv
            ...
            mListAdapter.setOnItemChildClickListener { adapter, view, position ->
                when(view.id){
                    ...
                    R.id.edit3_tv,R.id.edit4_left_tv,R.id.edit4_right_tv->ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_FUWUWANCHENG)
                    ...
                    R.id.delete3_tv,R.id.delete4_left_tv,R.id.delete4_right_tv->XPopup.Builder(requireActivity()).asConfirm("提示","是否确认删除") {
                        HomeRepository.delete<Any>("fuwuwancheng",listOf(mListAdapter.data[position][when(view.id){
                    ...
                    R.id.list_box3_fbl, R.id.list_box4_left_fbl, R.id.list_box4_right_fbl,->ArouterUtils.startFragment(CommonArouteApi.PATH_FRAGMENT_DETAILS_FUWUWANCHENG,map=mapOf("mId" to mListAdapter.data[position][when(view.id){
    ...
    private fun FuwuwanchengcommonListLayoutBinding.initSearch() {
    ...
    private val lists = mutableListOf<List<FuwuwanchengItemBean>>()

    //添加list集合到lists
    private fun addLists(list: List<FuwuwanchengItemBean>) {
        var lastList = mutableListOf<FuwuwanchengItemBean>()
        if (lists.isNotEmpty()) {
    ...
    private fun loadData(page:Int,searchParams:MutableMap<String,String>?=null){
        ...
        mIsBack.yes {
                mListModel.page("fuwuwancheng", requestParams)
        }.otherwise {
                mListModel.list("fuwuwancheng", requestParams)
        }
    }
}