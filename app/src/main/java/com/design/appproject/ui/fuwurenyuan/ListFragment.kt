...
import com.design.appproject.ui.fuwurenyuan.ListFilterDialog
...
import com.design.appproject.databinding.FuwurenyuancommonListLayoutBinding
import com.design.appproject.logic.repository.HomeRepository
import com.design.appproject.logic.viewmodel.fuwurenyuan.ListModel
import com.design.appproject.ui.fuwurenyuan.ListAdapter
 ...
 * 服务人员列表页
 */
@Route(path = CommonArouteApi.PATH_FRAGMENT_LIST_FUWURENYUAN)
class ListFragment : BaseBindingFragment<FuwurenyuancommonListLayoutBinding>() {
    ...
    val queryIndex = Pair<String,String>("员工姓名","yuangongxingming")/*查询索引*/

    override fun initEvent() {
        setBarTitle("服务人员",mHasBack)
        setBarColor("#C6EBF1","black")
        binding.apply {
            addBtn.isVisible = (mIsBack && Utils.isAuthBack("fuwurenyuan","新增"))|| Utils.isAuthFront("fuwurenyuan","新增")
            addBtn.setOnClickListener {
                ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_FUWURENYUAN).navigation()
            }
            initSearch()
          ...
          mListAdapter.addChildClickViewIds(R.id.edit1_tv,R.id.edit2_left_tv,R.id.edit2_right_tv
            ...
            mListAdapter.setOnItemChildClickListener { adapter, view, position ->
                when(view.id){
                    ...
                    R.id.edit3_tv,R.id.edit4_left_tv,R.id.edit4_right_tv->ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_FUWURENYUAN)
                    ...
                    R.id.delete3_tv,R.id.delete4_left_tv,R.id.delete4_right_tv->XPopup.Builder(requireActivity()).asConfirm("提示","是否确认删除") {
                        HomeRepository.delete<Any>("fuwurenyuan",listOf(mListAdapter.data[position][when(view.id){
                    ...
                    R.id.list_box3_fbl, R.id.list_box4_left_fbl, R.id.list_box4_right_fbl,->ArouterUtils.startFragment(CommonArouteApi.PATH_FRAGMENT_DETAILS_FUWURENYUAN,map=mapOf("mId" to mListAdapter.data[position][when(view.id){
    ...
    private fun FuwurenyuancommonListLayoutBinding.initSearch() {
    ...
    private val lists = mutableListOf<List<FuwurenyuanItemBean>>()

    //添加list集合到lists
    private fun addLists(list: List<FuwurenyuanItemBean>) {
        var lastList = mutableListOf<FuwurenyuanItemBean>()
        if (lists.isNotEmpty()) {
    ...
    private fun loadData(page:Int,searchParams:MutableMap<String,String>?=null){
        ...
        binding.searchValueEt.text.toString().isNotNullOrEmpty().yes {
            params.put("yuangongxingming","%" + binding.searchValueEt.text.toString() + "%" )
        }.otherwise {
            params.remove("yuangongxingming")
        }
        val requestParams = mutableMapOf<String,String>()
        requestParams.putAll(params)
        searchParams?.let { requestParams.putAll(it) }
        mIsBack.yes {
                mListModel.page("fuwurenyuan", requestParams)
        }.otherwise {
                mListModel.list("fuwurenyuan", requestParams)
        }
    }
}